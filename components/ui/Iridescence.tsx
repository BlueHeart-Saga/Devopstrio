...
export default function Iridescence({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("WebGL not supported for Iridescence:", e);
      return;
    }
    ...
    // Style the canvas to fill the absolute container
    ...
    function resize() {
      if (!ctn) return;
      const w = ctn.offsetWidth || ctn.clientWidth || window.innerWidth;
      const h = ctn.offsetHeight || ctn.clientHeight || window.innerHeight;
  ...
  return <div ref={ctnDom} className={`w-full h-full relative overflow-hidden ${className}`} {...rest} />;
}
