...
export const LineSidebar = ({
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent<HTMLUListElement>) => {
      ...
      for (let i = 0; i < itemsList.length; i++) {
        ...
        const prox = ease(Math.max(0, 1 - distance / proximityRadius));
        targetsRef.current[i] = prox;
      }
      startLoop();
    },
  ...
  return (
    <nav
      className={`relative flex justify-start select-none${showMarker ? ' [padding-left:calc(var(--marker-length)+var(--marker-gap))]' : ''}${className ? ` ${className}` : ''}`}
      style={
        {
        ...
        {items.map((label, index) => {
          ...
          return (
            <li
              ...
              onMouseEnter={() => handleItemHover(index, label)}
              onClick={() => handleItemHover(index, label)}
              className={`relative cursor-pointer transition-all duration-300 before:absolute before:-inset-x-12 before:-inset-y-[6px] before:content-[''] ${tickClass}`}
            >
              {showMarker && (
                <span
                  aria-hidden="true"
