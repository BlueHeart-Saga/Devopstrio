...
export function IndustryCards({ industries }: IndustryCardsProps) {
  ...
  return (
    <section className="w-full py-24 bg-black border-b border-zinc-900/60 relative overflow-hidden">
      ...
      <div className="absolute top-[40%] right-[-10%] w-[30%] aspect-square bg-[radial-gradient(circle_at_center,rgba(244,63,94,0.01),transparent_70%)] pointer-events-none z-0" />
          ...
          {industries.map((item, idx) => {
            ...
            return (
              <Reveal key={idx} delay={idx * 0.05} className="h-full">
                <Link
                  href={href}
                  className="group flex flex-col justify-between h-full bg-zinc-950/20 border border-zinc-900 hover:border-rose-500/35 hover:bg-zinc-900/10 rounded-[24px] p-6 md:p-8 transition-all duration-300 hover:scale-[1.01] text-left relative overflow-hidden cursor-pointer hover:shadow-[0_8px_32px_rgba(0,0,0,0.5)]"
                >
