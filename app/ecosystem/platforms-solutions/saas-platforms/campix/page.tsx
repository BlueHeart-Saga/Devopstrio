...
export default function CampixProductPage() {
  ...
  const features = [
    { title: "Traffic Splitting", desc: "Automate A/B testing on landing page components and redirect visits based on conversion parameters." },
    { title: "Spend Auditing", desc: "Collect expenditure lists from Google, Meta, and LinkedIn ads into a centralized ledger." },
    { title: "Analytics Aggregator", desc: "Merge click streams, lead submissions, and product checkout histories automatically." },
    { title: "Channel Attribution", desc: "Identify high-performing visitor flows using multi-touch and first-touch attribution equations." }
  ];

  const techStack = [
    { name: "React / Next.js", desc: "Interactive analytics layout with real-time graph rendering components." },
    { name: "Python / FastAPI", desc: "Fast data processor querying and aggregating external advertising APIs." },
    { name: "PostgreSQL", desc: "Time-series optimized tables tracking marketing events and conversions." }
  ];

  const faqs = [
    { q: "How does Campix fetch spend figures?", a: "We link with Google Ads, Meta Graph API, and LinkedIn Marketing Developer APIs to pull costs automatically." },
    { q: "Is traffic splitting cookie-based?", a: "We support both cookie-based routing and edge-level redirects (e.g. via Cloudflare Workers) to minimize latency." },
    { q: "Can we export the data to external warehouses?", a: "Yes, you can configure scheduled sync pipelines to push raw tables directly to Snowflake or Databricks." }
  ];
    ...
    <main className="min-h-screen bg-black text-white pt-16 font-sans overflow-x-hidden selection:bg-rose-500 selection:text-white">
        ...
        <div className="max-w-5xl mx-auto w-full px-12 xl:px-8 text-center relative z-10">
          ...
          <Reveal delay={0.1}>
            ...
            <p className="text-xl md:text-2xl font-bold text-rose-500 mb-8 max-w-2xl mx-auto">
              Campaign & Marketing Operations Platform
            </p>
          </Reveal>
          <Reveal delay={0.2} className="max-w-3xl mx-auto mb-10">
            <p className="text-zinc-200 text-sm md:text-base lg:text-lg leading-relaxed font-medium">
              Unify campaign operations, automate traffic splitting experiments, track multi-channel spend, and map conversion attributions.
            </p>
          </Reveal>
      ...
      <section className="w-full py-24 bg-black text-center relative">
        <div className="max-w-3xl mx-auto px-12 xl:px-8">
          <h2 className="text-2xl md:text-4xl font-bold mb-6">Deploy Campix in Your Workspace</h2>
