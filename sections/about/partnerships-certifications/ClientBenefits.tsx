...
export function ClientBenefits() {
  ...
  return (
    <section className="py-10 sm:py-14 bg-[#030303] text-white relative overflow-hidden z-10 font-sans">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {points.map((point, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <div className="relative group p-8 rounded-2xl border border-zinc-800 bg-zinc-950/40 hover:bg-zinc-900/60 transition-all duration-300 h-full flex flex-col">
                <div className="w-12 h-12 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center mb-6 group-hover:border-rose-500/50 transition-colors">
                  <span className="text-base font-bold text-rose-500">0{i + 1}</span>
                </div>
                <h3 className="text-xl font-bold text-white mb-3 font-sans">{point.title}</h3>
