...
import { BookReaderModal, AnnouncementReport } from "@/components/ui/BookReaderModal";
...
export function CultureAlbumsSection() {
  const [email, setEmail] = useState("");
  const [albums, setAlbums] = useState<CultureAlbum[]>([]);
  ...
  useEffect(() => {
    fetch("/api/culture-albums")
      .then((res) => res.json())
      .then((data) => {
        const list = Array.isArray(data) ? data : [];
        const activeList = list.filter((item: CultureAlbum) => item.status === "active");
        setAlbums(activeList.length > 0 ? activeList : list);
      })
      .catch((err) => console.error("Failed to load culture albums", err))
  ...
  // Auto-cycle albums every 6 seconds (pauses on hover or reader open)
  useEffect(() => {
    if (albums.length <= 1 || isHovered || isReaderOpen) {
  ...
  }, [albums.length, isHovered, isReaderOpen, goToNext]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (email) {
      alert(`Thank you! ${email} has been subscribed to Devopstrio Culture Updates.`);
      setEmail("");
    }
  };
  ...
  return (
    <section 
      className="w-full py-16 bg-[#030303] text-white relative overflow-hidden font-sans border-t border-zinc-900"
    ...
    >
      ...
      <BookReaderModal
        ...
        report={current as AnnouncementReport}
      />

      <div className="max-w-7xl mx-auto w-full px-6 sm:px-8 xl:px-8">
        <Reveal>
          <div className="relative w-full rounded-3xl border border-zinc-800 bg-[#08080a] p-6 sm:p-10 lg:p-12 overflow-hidden shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-8 sm:gap-12 group/card">
            ...
            <div className={`flex-1 z-10 w-full transition-opacity duration-300 ${isFading ? "opacity-30 translate-y-1" : "opacity-100 translate-y-0"}`}>
              <div className="pb-6 mb-6 border-b border-zinc-800/80">
                <span className="text-xs font-semibold uppercase tracking-[0.25em] text-rose-500 mb-2 block font-sans">
                  CULTURE & PEOPLE LOOKBOOK
                </span>
                <h2 className="text-3xl sm:text-4xl lg:text-5xl font-semibold tracking-tight text-white mb-2 leading-tight font-sans">
                  {current.titlePrefix} <span className="text-rose-500 font-semibold">{current.titleHighlight}</span>{current.titleSuffix ? `, ${current.titleSuffix}` : ""}
                </h2>
              ...
              {/* Newsletter Sub-Form */}
              <div>
                <span className="block text-xs font-semibold text-zinc-400 mb-2.5 font-sans">
                  {current.formTitle || "Stay updated with Devopstrio Culture & Career news"}
                </span>
                <form onSubmit={handleSubmit} className="flex max-w-md items-center relative">
                  ...
                  <input
                    type="email"
                    required
                    placeholder="Enter your email for culture updates"
                    value={email}
              ...
              {/* Multi-Album Controls & Indicators */}
              {albums.length > 1 && (
                    ...
                    {albums.map((_, idx) => (
                      <button
                        ...
                        className={`h-1.5 rounded-full transition-all duration-300 cursor-pointer ${
                          currentIndex === idx 
                            ? "w-7 bg-rose-600 shadow-[0_0_10px_rgba(244,63,94,0.6)]" 
                            : "w-2 bg-zinc-700 hover:bg-zinc-500"
                        }`}
                        title={`Culture Album ${idx + 1}`}
                      />
                    ))}
                  </div>
            ...
            <div 
              ...
              className={`relative w-full max-w-[220px] aspect-[3/4] rounded-2xl p-6 flex flex-col justify-between shadow-[0_25px_60px_rgba(0,0,0,0.95)] overflow-hidden mx-auto transition-all duration-300 bg-[#0E0F14] border border-zinc-800 hover:border-rose-500/60 hover:scale-105 cursor-pointer group select-none flex-shrink-0 ${
                isFading ? "opacity-30 scale-95" : "opacity-100 scale-100"
              }`}
              title="Click to Open Culture Lookbook PDF"
            >
              ...
              <div className="flex justify-between items-start relative z-10">
                <span className="text-[10px] font-bold text-rose-400 tracking-wider uppercase font-sans">
                  {current.reportType || "CULTURE ALBUM"}
                </span>
              ...
              <div className="my-auto relative z-10">
                <h3 className="text-base font-bold tracking-tight text-white leading-snug font-sans uppercase">
                  {current.coverTitleLine1 || "LIFE & CULTURE"}<br />
                  <span className="text-rose-400">{current.coverTitleLine2 || "LOOKBOOK"}</span>
                </h3>
              ...
              <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-2 transition-opacity z-20">
                ...
                <span className="text-xs font-bold text-white uppercase tracking-wider font-sans">Click to Read Album</span>
              </div>
            </div>
...
export default CultureAlbumsSection;
