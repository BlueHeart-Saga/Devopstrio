...
export function DownloadsLibrarySection({
  ...
  const displayItems = filteredResources || dbResources;
  return (
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">
            Searchable index of all downloadable files across corporate, practice, platform, and technical assets ({displayItems.length} assets available).
          </p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-zinc-800/60">
              {displayItems.slice(0, 15).map((item) => (
                <tr key={item.id} className="hover:bg-zinc-800/40 transition-colors group">
                  ...
                  <td className="py-4 px-4">{getTypeBadge ? getTypeBadge(item.type) : <span className="px-2 py-0.5 text-[10px] font-bold bg-zinc-800 text-zinc-300 rounded">{item.type || "PDF"}</span>}</td>
