...
export function DetailedServices({ hideExploreButton = false }: DetailedServicesProps) {
  ...
  const categories = useMemo(() => {
    ...
    const allCats = Array.from(new Set(integrations.map(i => i.category))).sort();
  ...
  return (
    <section className="w-full py-24 bg-black text-white relative" id="detailed-services">
      ...
      />
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] bg-rose-600/5 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-10 w-[500px] h-[500px] bg-rose-900/5 rounded-full blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto w-full px-6 md:px-12 relative z-10">
        ...
        <div className="bg-[#050505]/60 border border-zinc-900/80 rounded-3xl overflow-hidden shadow-2xl backdrop-blur-xl">
          
          {/* HEADER / SEARCH AREA */}
          <div className="p-6 md:p-8 border-b border-zinc-900/80 bg-zinc-950/20 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="relative w-full max-w-lg">
              <FiSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-rose-500 text-lg pointer-events-none" />
              <input
                type="text"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] min-h-[600px] lg:h-[750px]">
            ...
            {/* MAIN CONTENT */}
            <main className="p-6 md:p-8 overflow-y-auto lg:h-full custom-scrollbar bg-black/40">
                ...
                <div className="mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {popularIntegrations.map((item, idx) => (
                      <div
                        key={idx}
                        className="group relative bg-zinc-950/40 hover:bg-zinc-950/85 border border-zinc-900 hover:border-rose-500/30 rounded-2xl p-6 transition-all duration-300 cursor-default"
                      >
                        <div className="flex items-center gap-4 mb-4">
                          <div className="w-12 h-12 bg-rose-500/10 border border-rose-500/20 text-rose-500 rounded-xl flex items-center justify-center text-xl transition-all duration-300 group-hover:scale-110">
                            {item.icon}
                          </div>
                          <div>
              ...
              {/* Grouped Categories or Single Category View */}
              {activeCategory === "All" && searchQuery === "" ? (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredIntegrations.map((item, idx) => (
                      <div
                        key={idx}
                        className="group bg-[#080808]/80 hover:bg-zinc-950/80 border border-zinc-900/60 hover:border-zinc-800/80 rounded-2xl p-5 transition-all duration-300 flex flex-col justify-between cursor-default"
                      >
                        <div>
