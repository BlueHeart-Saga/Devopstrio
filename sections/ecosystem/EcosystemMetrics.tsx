        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
          {metrics.map((met, idx) => (
            <Reveal key={met.label} delay={idx * 0.04} className="h-full">
              <div className="group/card flex flex-col justify-center h-full bg-zinc-950/20 border border-white/5 hover:border-rose-500/20 hover:bg-zinc-950/40 rounded-2xl p-6 transition-all duration-300 relative overflow-hidden backdrop-blur-sm min-h-[140px] text-center">
                {/* Glow on hover */}
                ...
                <div className="text-3xl md:text-4xl lg:text-5xl font-semibold text-rose-500 group-hover/card:scale-105 transition-transform duration-300 mb-2 drop-shadow-[0_2px_10px_rgba(244,63,94,0.15)]">
                  {met.value}
                </div>
